 ...
 * @since 3.0
 */
class plgVmPaymentGercpay extends vmPSPlugin
{
    // instance of class
    public static $_this = false;

    /**
     * plgVmPaymentGercpay constructor.
     * @param $subject
     * @param $config
     * @since
     */
    public function __construct(&$subject, $config)
    {
        ...
        $this->_tablepkey  = 'id'; //virtuemart_gercpay_id';
        $this->_tableId    = 'id'; //'virtuemart_gercpay_id';
    ...
    public function plgVmConfirmedOrder($cart, $order)
    {
        ...
        include_once(__DIR__ . '/GercPay.cls.php');
        ...
        $gercpay = new GercPay();
        $gercpay->setSecretKey($method->gercpay_secret_key);
        ...
        $formFields = array(
            'operation'    => 'Purchase',
            'merchant_id'  => $method->gercpay_merchant_account,
            'amount'       => round($orderDetails->order_total, 2),
            'order_id'     => $cart->order_number . GercPay::ORDER_SEPARATOR . time(),
            'currency_iso' => $method->gercpay_currency,
        ...
        $gercpayArgsArray = array();
        foreach ($formFields as $key => $value) {
            if (is_array($value)) {
                foreach ($value as $v) {
                    $gercpayArgsArray[] = "<input type='hidden' name='{$key}[]' value='$v'/>";
                }
            } else {
                $gercpayArgsArray[] = "<input type='hidden' name='$key' value='$value'/>";
            }
        }

        $html = '<form action="' . Gercpay::URL . '" method="post" id="gercpay_payment_form">' .
            implode('', $gercpayArgsArray) . '</form>' .
            '<div><img src="/plugins/vmpayment/gercpay/assets/images/loader.gif"
  				 width="50px" style="margin:20px 20px;"></div>' .
            "<script> setTimeout(function() {
                 document.getElementById('gercpay_payment_form').submit();
             }, 500);
            </script>";
    ...
    public function plgVmOnPaymentResponseReceived(&$html)
    {
        ...
        if (!class_exists('GercPay')) {
            require(__DIR__ . '/GercPay.cls.php');
        }

        list($order_id, ) = explode(GercPay::ORDER_SEPARATOR, $data['orderReference']);
        ...
        $gercpay = new GercPay();
        $gercpay->setSecretKey($method->gercpay_secret_key);
        $response = $gercpay->isPaymentValid($data);
        ...
        if ($response === true && isset($data['type'])) {
            ...
            if ($data['type'] === GercPay::RESPONSE_TYPE_PAYMENT) {
                $orderStatus = $method->gercpay_status_success;
            } elseif ($data['type'] === GercPay::RESPONSE_TYPE_REVERSE) {
                $orderStatus = $method->gercpay_status_refunded;
            }
            ...
            if ($orderStatus) {
                ...
                $orderitems['comments'] = 'GercPay ID: ' . $order_id . ' Ref ID: ' . $data['transactionId'];
    ...
    public function plgVmOnUserPaymentCancel()
    {
        ...
        require(__DIR__ . '/GercPay.cls.php');
        list($order_id, ) = explode(GercPay::ORDER_SEPARATOR, $data['order_id']);
        $order = new VirtueMartModelOrders();
    ...
    public function getTableSQLFields()
    {
        $SQLfields = array(
            ...
            'tax_id' => 'smallint(1)',
            'gercpay_custom' => 'varchar(255)',
            'gercpay_method' => 'varchar(200)',

            'gercpay_response_mc_gross' => 'decimal(10,2)',
            'gercpay_response_mc_currency' => 'char(10)',
            'gercpay_response_invoice' => 'char(32)',
            'gercpay_response_protection_eligibility' => 'char(128)',
            'gercpay_response_payer_id' => 'char(13)',
            'gercpay_response_tax' => 'decimal(10,2)',
            'gercpay_response_payment_date' => 'char(28)',
            'gercpay_response_payment_status' => 'char(50)',
            'gercpay_response_pending_reason' => 'char(50)',
            'gercpay_response_mc_fee' => 'decimal(10,2)',
            'gercpay_response_payer_email' => 'char(128)',
            'gercpay_response_last_name' => 'char(64)',
            'gercpay_response_first_name' => 'char(64)',
            'gercpay_response_business' => 'char(128)',
            'gercpay_response_receiver_email' => 'char(128)',
            'gercpay_response_transaction_subject' => 'char(128)',
            'gercpay_response_residence_country' => 'char(2)',
            'gercpay_response_txn_id' => 'char(32)',
            'gercpay_response_txn_type' => 'char(32)', //The kind of transaction for which the IPN message was sent
            'gercpay_response_parent_txn_id' => 'char(32)',
            'gercpay_response_case_creation_date' => 'char(32)',
            'gercpay_response_case_id' => 'char(32)',
            'gercpay_response_case_type' => 'char(32)',
            'gercpay_response_reason_code' => 'char(32)',
            'gercpayresponse_raw' => 'varchar(512)',
            'gercpay_fullresponse' => 'text',
        );
        return $SQLfields;
    }
    ...
    {
        return JFactory::getLanguage()->_('VMPAYMENT_GERCPAY_PAYMENT_DESCRIPTION') . ' ' .
    ...
    {
        if (in_array($currency, GercPay::ALLOWED_CURRENCIES, true)) {
    ...
    protected function displayLogos($logo_list): string
    {
        ...
        if (! empty($logo_list)) {
            $url = JURI::root() . 'plugins/vmpayment/gercpay/';
